...
var MAC={
	...
	'History': {
		...
		'Clear': function(){
			...
			dialog.alert({
        title: "--==播放记录==--",
        msg: '<li class="history-item history-item-center"><a>已清空播放记录</a></li>',
        buttons: ['关闭']
      }, function (ret) {
		...
		'Show': function(){
			dialog.alert({
        title: "--==播放记录==--",
        msg: MAC.History.Create("history"),
        buttons: ['清空记录','关闭']
      }, function (ret) {
        if (ret.buttonIndex == 1) {
          MAC.History.Clear();
      ...
      if (jsondata.length > 0) {
        for (var $i = 0; $i < jsondata.length; $i++) {
          html += '<li class="history-item"><a href="' + jsondata[$i].link + '">' +($i+1)+'. ' +jsondata[$i].name + '</a></li>';
        }
      } else {
        html += '<li class="history-item history-item-center"><a>暂无播放记录</a></li>';
      }
      return html;
